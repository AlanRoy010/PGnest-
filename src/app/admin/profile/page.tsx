...
export default function AdminProfilePage() {
  ...
  const handleSave = async () => {
    ...
    const { error } = profile
      ...
      : await supabase.from("profiles").insert({ id: user.id, ...payload, role: "admin" });
    setSaving(false);
    if (error) { toast.error(error.message); return; }
    setLocalProfile((prev) => prev
      ? { ...prev, ...payload }
      : { id: user.id, ...payload, role: "admin", email: authEmail, avatar_url: null, is_verified: false, created_at: "", updated_at: "" }
    );
  ...
  const initial = profile?.full_name
    ...
    : displayEmail?.[0]?.toUpperCase() || "A";

  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-6">
        <h1 className="font-display text-2xl font-semibold text-[#1c1917]">My Profile</h1>
        <p className="text-sm text-[#78716c] mt-1">View and manage your admin account details</p>
      </div>

      {/* Avatar + name */}
      <div className="bg-white border border-[#e7e5e4] rounded-2xl p-6 mb-4 flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl bg-[#ea6c0a] flex items-center justify-center text-2xl font-semibold text-white flex-shrink-0">
          {initial}
        </div>
        <div>
          ...
          <div className="text-sm text-[#78716c] mt-0.5">Administrator</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#e7e5e4] rounded-2xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className={readCls}>Administrator</div>
          </div>
        </div>
