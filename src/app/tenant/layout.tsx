...
import { Search, BookOpen, Shield, LogOut, UserCircle, Menu, X } from "lucide-react";
    ...
    <div style={{ background: D.bg, minHeight: "100vh" }}>
      ...
      <nav style={{
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  color: active ? D.teal : locked ? "rgba(14,17,32,0.35)" : D.textDim,
        ...
        <div className="hidden md:flex" style={{ gap: 8, alignItems: "center" }}>
          {isLoggedIn ? (
            <>
              <Link
                ...
                style={{
                  ...
                  textDecoration: "none", fontSize: 13, color: D.text, fontWeight: 500,
                }}
              >
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 99, background: "none", border: `1px solid ${D.border}`, color: D.textDim, fontSize: 13, cursor: "pointer" }}
              >
            ...
            <>
              <Link href="/login" style={{ padding: "7px 16px", fontSize: 13, color: D.textDim, textDecoration: "none", borderRadius: 99 }}>
                Sign In
              </Link>
              <Link href="/signup" style={{ padding: "8px 18px", borderRadius: 99, background: D.teal, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
                <UserCircle style={{ width: 14, height: 14, display: "inline", marginRight: 6, verticalAlign: "middle" }} />
                Get Started
              </Link>
            </>
          )}
      ...
      {mobileOpen && (
        <div className="md:hidden" style={{
          ...
          background: "rgba(238,238,238,0.97)", backdropFilter: "blur(20px)",
          borderBottom: `1px solid ${D.border}`, padding: "12px 24px 20px",
        }}>
          {NAV_ITEMS.map(item => {
            const active = pathname.startsWith(item.href);
            const locked = item.requiresAuth && !isLoggedIn;
            return (
              <Link
                ...
                style={{
                  ...
                  textDecoration: "none", color: active ? D.teal : D.textDim,
          ...
          <div style={{ borderTop: `1px solid ${D.border}`, marginTop: 12, paddingTop: 12 }}>
            {isLoggedIn ? (
              <button onClick={handleSignOut} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: D.textDim, background: "none", border: "none", cursor: "pointer", padding: "11px 14px" }}>
                <LogOut style={{ width: 16, height: 16 }} /> Sign out
              </button>
            ) : (
              <Link href="/login" style={{ display: "block", padding: "11px 14px", fontSize: 14, color: D.textDim, textDecoration: "none" }}>Sign In</Link>
            )}
          </div>
        </div>
      )}
